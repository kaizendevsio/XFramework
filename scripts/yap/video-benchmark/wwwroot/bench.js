window.runBench = async (asyncOnly = false, height = 1080, fps = 30) => {
 ...
 const source=document.createElement('canvas'); source.width=Math.round(height*16/9);source.height=height;
 ...
 navigator.mediaDevices.getUserMedia=async()=>source.captureStream(fps);
 ...
 const timer=setInterval(()=>{const c=source.getContext('2d'); c.fillStyle='#124';c.fillRect(0,0,1920,1080);for(let i=0;i<400;i++){c.fillStyle=`hsl(${(i*17+tick)%360} 80% 50%)`;c.fillRect((i*53+tick*7)%1920,(i*97)%1080,48,48);}tick++;},1000/fps);
 try{await pipeline.initEncoder('h264',source.width,height,height>=1440?9750:5700,fps,2);pipeline.addRemote('loop',output,'h264',host);pipeline.attachPreview(preview);await pipeline.startCapture(host,{});await new Promise(r=>setTimeout(r,12000));pipeline.stopCapture();while(busy)await new Promise(r=>setTimeout(r,10));
 const summary=a=>{a.sort((a,b)=>a-b);return {n:a.length,p50:a[Math.floor(a.length*.5)],p95:a[Math.floor(a.length*.95)]};};
 return {captured:pipeline.captureCount,encoderStats:pipeline.getStats(),requested:{height,fps},encodedFps:encoded/12,renderedFps:rendered/12,encoded,rendered,dropped,errors,latency:summary(latencies),cryptoAndWsAndInterop:summary(cryptoTransport),bytes:summary(sizes),softwareFallback:pipeline.remotes.get('loop').software,config:pipeline.config};
