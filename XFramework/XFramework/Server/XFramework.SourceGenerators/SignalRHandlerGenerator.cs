...
public class SignalRHandlerGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        foreach (var model in models)
        {
            ...
            codeBuilder.AppendLine("                         {");
            codeBuilder.AppendLine("                             public void Handle(HubConnection connection, IMediator mediator, ILogger<BaseSignalRHandler> logger, MetricsMonitor metricsMonitor)");
            codeBuilder.AppendLine("                             {");
            codeBuilder.AppendLine($"                                 HandleRequestQuery<GetList<{model}>, PaginatedResult<{model}>>(connection, mediator, logger, metricsMonitor);");
            codeBuilder.AppendLine("                             }");
            codeBuilder.AppendLine("                         }");
            ...
            codeBuilder.AppendLine("                             public void Handle(HubConnection connection, IMediator mediator, ILogger<BaseSignalRHandler> logger, MetricsMonitor metricsMonitor)");
            codeBuilder.AppendLine("                             {");
            codeBuilder.AppendLine($"                                 HandleRequestQuery<Get<{model}>, {model}>(connection, mediator, logger, metricsMonitor);");
            codeBuilder.AppendLine("                             }");
            codeBuilder.AppendLine("                         }");
            ...
            codeBuilder.AppendLine("                             public void Handle(HubConnection connection, IMediator mediator, ILogger<BaseSignalRHandler> logger, MetricsMonitor metricsMonitor)");
            codeBuilder.AppendLine("                             {");
            codeBuilder.AppendLine($"                                 HandleRequestCmd<Create<{model}>, {model}>(connection, mediator, logger, metricsMonitor);");
            codeBuilder.AppendLine("                             }");
            codeBuilder.AppendLine("                         }");
            ...
            codeBuilder.AppendLine("                             public void Handle(HubConnection connection, IMediator mediator, ILogger<BaseSignalRHandler> logger, MetricsMonitor metricsMonitor)");
            codeBuilder.AppendLine("                             {");
            codeBuilder.AppendLine($"                                 HandleRequestCmd<Replace<{model}>, {model}>(connection, mediator, logger, metricsMonitor);");
            codeBuilder.AppendLine("                             }");
            codeBuilder.AppendLine("                         }");
            ...
            codeBuilder.AppendLine("                             public void Handle(HubConnection connection, IMediator mediator, ILogger<BaseSignalRHandler> logger, MetricsMonitor metricsMonitor)");
            codeBuilder.AppendLine("                             {");
            codeBuilder.AppendLine($"                                 HandleRequestCmd<Patch<{model}>, {model}>(connection, mediator, logger, metricsMonitor);");
            codeBuilder.AppendLine("                             }");
            codeBuilder.AppendLine("                         }");
            ...
            codeBuilder.AppendLine("                             public void Handle(HubConnection connection, IMediator mediator, ILogger<BaseSignalRHandler> logger, MetricsMonitor metricsMonitor)");
            codeBuilder.AppendLine("                             {");
            codeBuilder.AppendLine($"                                 HandleRequestCmd<Delete<{model}>, {model}>(connection, mediator, logger, metricsMonitor);");
            codeBuilder.AppendLine("                             }");
            codeBuilder.AppendLine("                         }");
