...
{
    public class CreateIdentityWalletHandler : BaseSignalRHandler, ISignalREventHandler
    {
        public void Handle(HubConnection connection, IMediator mediator)
        {
            ...
            connection.On<string, string, StreamFlowTelemetryBO>(GetType().Name.Replace("Handler", string.Empty),
                async (data, message, telemetry) =>
                {
                    ...
                    try
                    {
                        var r = data.AsMediatorCmd<CreateIdentityWalletRequest, CreateIdentityWalletCmd>();
