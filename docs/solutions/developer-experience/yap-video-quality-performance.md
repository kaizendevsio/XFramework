...
## 1.3.63 - software decoder fallback through 1440p
...
The panel separates requested quality, camera-reported settings, encoder configuration, measured capture/encoded FPS and per-stream received/rendered FPS. It includes encoded/received payload bitrate, encoder/send/decoder queue depth, pending decoder outputs, capture-pressure drops and decoder resets. Received FPS counts complete frames passed to the browser pipeline, before decoder gating; rendered FPS counts successful canvas draws, not physical display refreshes. Codec timings are the last measured submission-to-output duration, not network or end-to-end latency. Bitrates exclude fragmentation/encryption/transport overhead. No participant/device identifiers, frame contents or keys are shown.
