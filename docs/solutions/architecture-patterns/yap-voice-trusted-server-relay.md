...
### Rate control across a resume, 27 September 2026

With phase 1's congestion control and phase 2's resumes together, a resume replaces the send path, not just the socket. `BoltMediaService.SuspendTransportAsync` stops the pacer and rate loop of the lost transport (the pacer sends on its client, and the controller's delay, capacity and congestion history describe the old path) and keeps only the controller. The resumed transport's `SendRateController.Resume` starts from half the old path's *stable* rate (a slow average over calm windows only, so neither a probe's peak nor the cuts the outage caused), never below the call's start tier; the camera's ladder is placed to fit before the first keyframe, which is requested at once and gated by the new pacer. A sender whose only receiver went silent for 5 s and comes back calm restarts that path the same way. Relay congestion reports (`0x27`) and heartbeats (`0x0E`) are wired in `CreateMediaClient`, so every transport gets both; a congestion report also counts as inbound liveness.

"Poor connection" has one source: `CallLinkMonitor` combines heartbeat silence and round trip with the rate controller's verdict (`BoltMediaService.SendPathPoor`: its queuing delay at or above its own `HighDelayMs`, or video suspended for bandwidth), held 3 s on both edges. The controller's verdict can only make a call "Poor connection"; "Reconnecting" is decided by silence alone.

`Yap:Calls:RelaySocketSendBufferBytes` (default 0, off) caps `SO_SNDBUF` on call sockets, bounding TCP's bytes in flight where the relay owns the phone's bottleneck leg. Behind an ingress proxy it has no effect on the phone's leg, and a fixed cap is a throughput ceiling on long paths, so it stays opt-in.
