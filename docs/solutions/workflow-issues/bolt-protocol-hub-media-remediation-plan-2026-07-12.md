...
**Status:** In progress - Phase 0 code and fail-closed rollout automation passed current local checks and independent review; the first committed Linux CI run, one-time recovery bootstrap, and live staged evidence remain open
...
|---:|---|---|---|
| 1 | Phase 0 - Immediate containment | `Contained; current local checks passed; CI/live gates pending` | Commit and run the Linux CI gate, provision the sealed watchdog bootstrap, execute the staged xeon-dev rollout, and collect the required live evidence. |
...
The Phase 0 branch contains only the minimum wire-compatible client containment required for bounded registration acknowledgement, registration-attempt transport and acknowledgement-buffer cleanup, baseline pending-call accounting, and process-lifetime transport-health watermarks. Registration cleanup is the inseparable failure path of source-audit finding M1: a timed-out registration must not leak its socket while retrying. The broader client lifecycle and ownership rewrite is excluded from this phase. The unread-stream, send-completion, large-response-affinity, handler-lifecycle, custom-frame ownership, retained pooled-chunk, zero chunk-size, general failed-pool-member retirement/reconnect cleanup, connection retirement, and buffer-lifetime findings remain open for Phases 3 and 4.
...
|---|---|---|---|
| P0-A | Containment code: `Enforce`, reserved identity binding, temporary quotas, 8 MiB ceiling, default-deny authorization, socket lifetime, redaction, Media kill switch, single replica | Implemented | Bolt tests, negative identity/authorization tests, configuration tests, container build, independent containment review. |
| P0-B | Deployment preflight: exact `repository@sha256:digest` pins, approved repository mapping, signed build provenance, run-scoped full-stack manifests, effective/runtime Kestrel listeners, resolved private-key mount isolation, typed non-executing env parsing, internal and real public-hostname certificate validation, fatal CA installation, exact quotas, and one running Hub | Implemented; live evidence pending | Adversarial verifier tests pass locally. Registry, provenance, listener, mount, TLS, and runtime evidence must be produced by the live run. |
...
| P0-E | Authenticated synthetics and observation | Implemented; protected inputs pending | HTTPS token acquisition, WSS/RPC/pub-sub/durable/expiry/plaintext/marker probes and canary observation are automated. Provision the dedicated principal and retained-store query inputs. |
...
6. Run the complete synthetic suite, including offline replay and Redis interruption, and verify that its unique token marker is absent from application, proxy, Seq, and trace storage.
...
|---|---|---|---|
| P0-R1 | High | A mutable 40-character tag or unrelated repository digest could satisfy the deployment authorization check. | Implemented locally: approved repository mapping, registry-confirmed digests, signed provenance, and run-scoped state are mandatory. Live evidence pending. |
| P0-R2 | High | The full workflow deploys every service at once and lacks a Hub-to-canary-to-synthetic gate and fail-closed recovery. | Implemented locally: staged promotion and forced fail-closed recovery are mandatory. Live evidence pending. |
| P0-R3 | Medium | Compose validation checked `ASPNETCORE_URLS` rather than effective Kestrel endpoints. | Implemented locally with named Kestrel and actual runtime listener/socket verification. |
| P0-R4 | Medium | Alternate secret aliases or bind mounts could expose the Hub private key to clients. | Implemented locally with resolved path/inode and parent-directory isolation checks. |
| P0-R5 | Medium | TLS evidence validated only the internal `bolt-hub` name, not the published hostname. | Implemented locally with internal/public names and real DNS/TLS routing checks. Live evidence pending. |
...
| P0-R10 | High | Deployment failures collect diagnostics but do not execute a qualified rollback or stop Bolt traffic. Shared remote pin files can be overwritten by single-service deployment. | Implemented locally with run-scoped state, rollback drill, forced recovery, root-sealed LKG, and Hub stop fallback. Live evidence pending. |
| P0-R11 | High | Preflight does not prove the effective runtime boundary: actual sockets, peer plaintext rejection, real public DNS/routing, or resolved/symlinked parent mounts. | Implemented locally in runtime/TLS/operational verifiers. Live evidence pending. |
| P0-R12 | Medium | Current durable synthetic subscribes before publish and does not prove offline persistence, ordered reconnect replay, no redelivery, monotonic acknowledgement, or Redis interruption recovery. | Implemented locally, including bounded exact Redis ACK behavior and post-recovery durable probes. Live evidence pending. |
...
| P0-R14 | High | Existing workflows generate missing secrets but do not atomically rotate all credentials or IdentityServer registrations; a single-service deployment can create generation skew. | Implemented locally; legacy independent deployment is frozen and rotation is full-stack/run-scoped. Live evidence pending. |
| P0-R15 | Medium | The preflight claims bounded limits and one replica without validating every exact configured quota or the runtime Hub process count. | Implemented locally, including a separate per-principal pending-RPC ceiling and one-Hub runtime inventory. |
| P0-R16 | High | Exact image identity is not bound to the reviewed source or trusted builder. | Implemented locally with signed provenance binding digest, commit, workflow/builder, Dockerfile, and base images. Live attestations pending. |
| P0-R17 | High | Rotation does not define activation order, dual-generation validation, old-generation revocation, or convergence across staged restarts and HTTP validators. | Implemented and failure-tested locally. Live convergence/revocation evidence pending. |
| P0-R18 | Critical/High | Workflow-controlled root execution, mutable recovery components, circular first-run bootstrap, activation crash windows, stale synthetic leases, replaceable recovery locks, and non-durable LKG publication could bypass or strand fail-closed recovery. | Implemented locally with an operator-reviewed root-only staging bundle; descriptor-relative complete reads and atomic installation; a fixed root-owned, inode-stable deployment lock shared by root activation and recovery; bounded Docker escalation; a fixed operator-installed root helper/launcher/lease manager; narrow sudoers commands; root-only quarantine; fresh-lease first rollout; supervised synthetic heartbeats; an always-active watchdog timer; crash-durable sealing/pointer publication; and privileged adversarial recovery tests. Operator bootstrap and live evidence remain pending. |
| P0-R19 | High | Canary health used a stale server schema and generic service health, so real payloads failed validation while transient client send/receive failures could disappear between samples. | Implemented locally with the exact server/client snapshot schemas, shared loopback-only client transport health, process-lifetime fault/disconnect/reconnect watermarks, and strict observation tests. Live canary evidence remains pending. |
...
| Credential redaction | Partially verified | First Hub middleware removes query tokens from application request/telemetry surfaces. Proxy, ingress, server pre-middleware, and Seq log evidence is still required. |
...
| Staging acceptance | Automated; live run pending | Authenticated registration/RPC/pub-sub/durable/expiry, marker searches, observation, rotation, rollback, qualification, and forced recovery are implemented. Protected inputs and live evidence remain required. |
| Automated evidence | Current local checks passed; Linux CI pending | `Bolt.Tests`: 303 passed with Redis mandatory; IdentityServer: 21 passed; Core: 195 passed; `Bolt.Phase0Synthetics.Tests`: 39 passed with one Linux-only symbolic-link test skipped on Windows. All 19 Phase 0 Python suites passed 370 tests on Windows with 43 platform skips; the unchanged suites previously passed 370 tests on privileged Linux with zero skips. Actionlint passed all 17 workflows; 75 workflow Bash blocks, 39 embedded Bash heredocs, six Phase 0 shell files, and 35 Phase 0 Python files passed local static validation. The full Release solution build completed with 0 errors. The committed Ubuntu CI run, including a Bolt Hub container build, 40 synthetic tests, and one positive plus seven negative wrapper scenarios, remains pending. |
| Independent verification | Passed for current local scope; CI/live gates pending | Targeted security/correctness reviews found and drove fixes for health exposure, scope-shape bypass, send-loop failure, durable ACK, JWT expiry, large-RPC quota cleanup, recovery trust/durability, client failure watermarks, stream-close authorization, privileged bootstrap completeness, and lock-inode stability. The final recovery review and the current CI-definition re-review passed with no findings at any severity. The first committed Linux CI execution, operator bootstrap, and live staged evidence remain required. Deferred Phase 3/4 client findings remain open. |
...
- Access tokens do not appear in application, proxy, Seq, or telemetry logs.
...
Create an ADR for Bolt authentication bootstrap before implementation. The design must break the current circular dependency: obtaining the token required to connect to Bolt cannot itself require a Bolt call.
...
10. Ensure query-string access tokens are redacted from proxy, ASP.NET, Seq, and OpenTelemetry logs.
