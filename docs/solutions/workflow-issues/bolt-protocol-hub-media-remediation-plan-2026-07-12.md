...
2. Preflight the rendered base Compose plus digest override on the build controller for approved repositories, signed source/build provenance, exact quotas, one-Hub topology, typed env input, resolved mounts, and effective endpoints. Before candidate pulls or runtime mutation, require the deployment host to repeat those checks and exclusively authorize real DNS/interface ownership and TLS reachability.
