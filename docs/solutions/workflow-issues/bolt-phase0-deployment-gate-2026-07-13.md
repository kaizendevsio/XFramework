...
- The current local correction passes 446 tests across all 19 Phase 0 Python test files on Windows with 49 platform skips; the PR #352 committed privileged Ubuntu baseline passed all then-current 370 tests with zero skips. Checksum-verified actionlint v1.7.8 passes all workflow YAML files; workflow parsing, all Phase 0 Python files, and changed-line whitespace validation pass locally. A fresh privileged Ubuntu run is required for the new tests and Linux-only branches.
...
- The local correction uses indexed optional-health inspection for raw-fallback one-shot and operational formats. Runtime inspection requires exactly one process-unowned Docker embedded-DNS listener when `127.0.0.11` is configured, exactly one application listener on `127.0.0.1:8080`, exactly one on `0.0.0.0:8443`, the exact dual-stack runtime host publication, and the single configured wildcard TLS binding. Extra exposed ports are accepted only when Docker reports them as `null` and therefore unbound; any bound plaintext/extra port, loopback/duplicate host binding, or listener drift fails closed, and failed publication evidence remains `null` instead of echoing the expected value. The sealed runtime v2 evidence retains its existing redacted shape after these exact live checks. Regression fixtures reproduce the xeon-dev topology and each negative case. This correction is not yet merged, CI-validated on the target branch, or redeployed.
