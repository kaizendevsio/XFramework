...
Phase 0 containment and rollout corrections through PR [#358](https://github.com/kaizendevsio/XFramework/pull/358) are merged at `a14a76a24c999ace69401d447edbf1bd3017abe5`. The root bootstrap is installed on xeon-dev, its watchdog is enabled and active, and Bolt Hub remains stopped with no deployment lease or LKG pointer. Four first-attempt workflow runs have failed closed before lease acquisition or service mutation; the latest exposed an env-reader validation error when an unrelated opaque secret contained characters outside the typed path grammar. Phase 0 remains `Contained`, not `Verified`, until a complete staged run produces live authenticated WSS, rotation, rollback, qualification, and recovery evidence. Do not disable `RequireSecureTransport`, restore Audit mode, or deploy a pre-containment image to bypass this gate. Production promotion remains blocked until Phase 1 removes the shared service-signing trust boundary.
