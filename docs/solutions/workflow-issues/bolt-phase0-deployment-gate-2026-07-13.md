...
Phase 0 containment and rollout corrections through PR [#362](https://github.com/kaizendevsio/XFramework/pull/362) are merged at `827351d286f84fe8fa6baabc468002c03c243682`. The corrected root bootstrap is installed on xeon-dev, its watchdog is enabled and active, and Bolt Hub remains stopped with no deployment lease or LKG pointer. Eight first-attempt workflow runs have failed closed before lease acquisition or service mutation; the latest produced and cryptographically verified all 14 image attestations, then exposed an invalid cross-namespace uniqueness rule in the retained-evidence verifier before provenance authorization. Phase 0 remains `Contained`, not `Verified`, until a complete staged run produces live authenticated WSS, rotation, rollback, qualification, and recovery evidence. Do not disable `RequireSecureTransport`, restore Audit mode, or deploy a pre-containment image to bypass this gate. Production promotion remains blocked until Phase 1 removes the shared service-signing trust boundary.
