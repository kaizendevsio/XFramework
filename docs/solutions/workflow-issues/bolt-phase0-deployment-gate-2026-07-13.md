...
Phase 0 containment and rollout corrections through PR [#359](https://github.com/kaizendevsio/XFramework/pull/359) are merged at `5cc892aef037a40b49eb89fd98c68ed985212e0d`. The root bootstrap is installed on xeon-dev, its watchdog is enabled and active, and Bolt Hub remains stopped with no deployment lease or LKG pointer. Five first-attempt workflow runs have failed closed before lease acquisition or service mutation; the latest exposed workflow staging directly beneath the intentionally root-owned deployment boundary instead of inside the prepared runner-owned run directory. Phase 0 remains `Contained`, not `Verified`, until a complete staged run produces live authenticated WSS, rotation, rollback, qualification, and recovery evidence. Do not disable `RequireSecureTransport`, restore Audit mode, or deploy a pre-containment image to bypass this gate. Production promotion remains blocked until Phase 1 removes the shared service-signing trust boundary.
