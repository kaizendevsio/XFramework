...
Phase 0 containment and rollout corrections through PR [#363](https://github.com/kaizendevsio/XFramework/pull/363) are merged at `3a65e98cc56fa393c3b24e7fad6e363c6c5eb718`. The corrected root bootstrap is installed on xeon-dev, its watchdog is enabled and active, and Bolt Hub remains stopped with no deployment lease or LKG pointer. Nine first-attempt workflow runs have failed closed before lease acquisition or service mutation; the latest passed signed provenance for all 14 images, then exposed a build-controller versus deployment-host locality bug in publication authorization. Phase 0 remains `Contained`, not `Verified`, until a complete staged run produces live authenticated WSS, rotation, rollback, qualification, and recovery evidence. Do not disable `RequireSecureTransport`, restore Audit mode, or deploy a pre-containment image to bypass this gate. Production promotion remains blocked until Phase 1 removes the shared service-signing trust boundary.
...
- The current local correction passes 439 tests across all 19 Phase 0 Python test files on Windows with 49 platform skips; the PR #352 committed privileged Ubuntu baseline passed all then-current 370 tests with zero skips. Checksum-verified actionlint v1.7.8 passes all workflow YAML files; workflow parsing, the changed authorization step's Bash syntax, all 35 Phase 0 Python files, and changed-line whitespace validation pass locally. A fresh privileged Ubuntu run is required for the new tests and Linux-only branches.
