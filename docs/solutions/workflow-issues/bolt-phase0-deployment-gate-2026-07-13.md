...
Phase 0 containment and rollout corrections through PR [#357](https://github.com/kaizendevsio/XFramework/pull/357) are merged at `e09c9c9bea61bcdd70338276b1890be6929cdcfd`. The root bootstrap is installed on xeon-dev, its watchdog is enabled and active, and Bolt Hub remains stopped with no deployment lease or LKG pointer. Three first-attempt workflow runs have failed closed before lease acquisition or service mutation; the latest exposed a sudo-rs 0.2.8 command-argument wildcard incompatibility in the generated root-helper policy. Phase 0 remains `Contained`, not `Verified`, until a complete staged run produces live authenticated WSS, rotation, rollback, qualification, and recovery evidence. Do not disable `RequireSecureTransport`, restore Audit mode, or deploy a pre-containment image to bypass this gate. Production promotion remains blocked until Phase 1 removes the shared service-signing trust boundary.
