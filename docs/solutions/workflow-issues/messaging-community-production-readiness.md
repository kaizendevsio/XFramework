---
title: "Messaging and Community Services Production Readiness"
date: 2026-04-01
category: workflow-issues
module: XFramework.Messaging
problem_type: workflow_issue
component: service_object
severity: high
applies_when:
  - "Hardening Messaging and Community service endpoints, validators, DI registration, contracts, migrations, and OpenAPI exposure for production readiness"
tags: [messaging, community, production-readiness, endpoints, validation]
---

# Messaging & Community Services — Production Readiness (Groups A & B)

**Date:** 2026-04-01
**Status:** Approved
**Scope:** Quick fixes + new endpoints for both Messaging and Community services. Excludes real-time/push notifications (in progress separately).
...
**Fix:**
- Rename property to `ReceivedAt` across: `MessageDirect` entity, `MessageDirectConfiguration`, `MessagingService`, `UpdateMessageDirectRequest`, and any response contracts referencing it.
- EF migration required for column rename.

### A3. Pagination Bounds Validation

**Problem:** Messaging list validators don't consistently enforce PageSize/PageIndex bounds. Community already does.

**Fix:** Add to all Messaging paginated request validators:
...
**Fix:**
- Create `Community.Domain.Shared/Constants.cs` following the Messaging `Constants.cs` pattern
...
### B5. Messaging: Update Thread

**Endpoint:** `PATCH /api/threads/{id}` + `[BoltHandler]`
**Location:** `Messaging.Api/Features/Threads/Update/`
...
Add to `Messaging.Domain.Shared/Constants.cs`:
```
...
### B7. Messaging: Fix Reactions (Schema Change)
