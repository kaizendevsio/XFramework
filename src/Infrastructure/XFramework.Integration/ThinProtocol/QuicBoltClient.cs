...
/// <summary>
/// QUIC-based Bolt client. Each RPC opens its own bidirectional QUIC stream,
...
/// </summary>
public sealed class QuicBoltHubClient : IAsyncDisposable
{
    ...
    public QuicBoltHubClient(IPEndPoint serverEndPoint, string clientId, string clientName,
        BoltConfiguration config, ILogger logger)
    {
        ...
        string recipientId, string commandName, ReadOnlyMemory<byte> payload, CancellationToken ct = default)
    {
        ...
        var recipientHash = BoltHubCodec.Fnv1aHash(recipientId);
    ...
    private async Task ReadRpcResponsesAsync(QuicStream stream, CancellationToken ct)
    {
        ...
        try
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                if (BoltHubCodec.TryReadResponse(buffer.AsSpan(0, read), out var frame, out _))
                {
    ...
    private async Task HandleInboundStreamAsync(QuicStream stream, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if ((FrameType)buffer[0] == FrameType.Request &&
                BoltHubCodec.TryReadRequest(buffer.AsSpan(0, read), out var frame, out _))
            {
