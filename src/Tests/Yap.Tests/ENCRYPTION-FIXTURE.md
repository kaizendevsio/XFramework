...
Set `YAP_FIXTURE_LINK_DELAY_MS=1000` before starting the host to reproduce the interval between encrypted message delivery and attachment-link creation. The delay defaults to zero and is capped at five seconds. Recipients should show a skeleton during that interval and load the preview after the link event, without needing a page refresh. Encrypted edits use the real BFF action endpoint and preserve opaque ciphertext plus its updated sender device/revision.
