...
/// <summary>
/// Provides bounded channel-based message queueing for Bolt SignalR messaging.
...
/// - SingleReader: True (BoltProcessor is the only consumer)
/// - SingleWriter: False (multiple SignalR connections can write)
/// </remarks>
public sealed class BoltMessageQueue : IDisposable
{
    private readonly Channel<BoltMessage> _messageChannel;
    private readonly Channel<(Guid Id, TaskCompletionSource<BoltMessage> Tcs)> _methodCallChannel;
    ...
    /// <summary>
    /// Initializes a new instance of BoltMessageQueue with capacity from configuration.
    /// </summary>
    /// <param name="configuration">Bolt configuration. Uses QueueDepth if > 0, otherwise defaults to 10,000.</param>
    public BoltMessageQueue(BoltConfiguration configuration)
    {
        ...
        var messageOptions = new BoundedChannelOptions(capacity)
        {
            ...
            SingleReader = true,  // BoltProcessor is single reader
        ...
        _messageChannel = Channel.CreateBounded<BoltMessage>(messageOptions);
        _methodCallChannel = Channel.CreateBounded<(Guid, TaskCompletionSource<BoltMessage>)>(methodCallOptions);
    }

    /// <summary>
    /// Gets the reader for the message channel.
    /// </summary>
    public ChannelReader<BoltMessage> MessageReader => _messageChannel.Reader;

    /// <summary>
    /// Gets the writer for the message channel.
    /// </summary>
    public ChannelWriter<BoltMessage> MessageWriter => _messageChannel.Writer;

    /// <summary>
    /// Gets the reader for the method call channel.
    /// </summary>
    public ChannelReader<(Guid Id, TaskCompletionSource<BoltMessage> Tcs)> MethodCallReader => _methodCallChannel.Reader;

    /// <summary>
    /// Gets the writer for the method call channel.
    /// </summary>
    public ChannelWriter<(Guid Id, TaskCompletionSource<BoltMessage> Tcs)> MethodCallWriter => _methodCallChannel.Writer;
    ...
    /// </remarks>
    public async Task<bool> TryEnqueueMessageAsync(BoltMessage message, CancellationToken cancellationToken = default)
    {
        if (message == null)
        {
    ...
    public async Task<bool> TryEnqueueMethodCallAsync(Guid id, TaskCompletionSource<BoltMessage> tcs, CancellationToken cancellationToken = default)
    {
        if (tcs == null)
        {
