...
- Inventario's Escape compatibility handler closes an entity-picker popover inside its parent dialog through the library's pointer outside-dismiss path. It prevents the separate dialog Escape listener from discarding the draft. Keep this scoped to Inventario dialogs and use fingerprinted host assets so browsers receive updated CSS/JavaScript after deployment.
