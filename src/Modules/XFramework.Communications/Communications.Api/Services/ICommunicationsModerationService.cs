...
public sealed record CommunicationsModerationRuleMatch(
    Guid RuleId,
    string RuleName,
    string Action);

public interface ICommunicationsModerationService
{
    Task<IReadOnlyList<CommunicationsModerationRuleMatch>> EvaluateAsync(
        Guid tenantId,
        string text,
        CancellationToken ct = default);

    Task<Result<GetCommunicationsModerationRulesResponse>> GetRulesAsync(
        GetCommunicationsModerationRulesRequest request,
        CancellationToken ct = default);

    Task<Result<CommunicationsModerationRuleResponse>> CreateRuleAsync(
        CreateCommunicationsModerationRuleRequest request,
        CancellationToken ct = default);

    Task<Result<CommunicationsModerationRuleResponse>> UpdateRuleAsync(
    ...
    Task<Result<CommunicationsReportWorkflowResponse>> ReviewReportAsync(
        ReviewMessageReportRequest request,
        CancellationToken ct = default);
}

public sealed class CommunicationsModerationService(
    IDataContext dataContext,
    ICommunicationsRequestContextResolver requestContextResolver) : ICommunicationsModerationService
{
    public async Task<IReadOnlyList<CommunicationsModerationRuleMatch>> EvaluateAsync(
        Guid tenantId,
        string text,
        CancellationToken ct = default)
    {
        ...
        return rules
            ...
            .Select(rule => new CommunicationsModerationRuleMatch(rule.Id, rule.Name, NormalizeAction(rule.Action)))
            .ToList();
    }

    public async Task<Result<GetCommunicationsModerationRulesResponse>> GetRulesAsync(
        GetCommunicationsModerationRulesRequest request,
        CancellationToken ct = default)
    {
        ...
        return Result<GetCommunicationsModerationRulesResponse>.Success(new()
        {
            Items = rules.Select(ToRuleResponse).ToList()
        });
    }

    public async Task<Result<CommunicationsModerationRuleResponse>> CreateRuleAsync(
        CreateCommunicationsModerationRuleRequest request,
        CancellationToken ct = default)
    {
        var tenant = ResolveAdminTenant(request.Metadata);
        ...
        if (validation.Count > 0)
            return Result<CommunicationsModerationRuleResponse>.ValidationError(validation);
        ...
        var duplicate = await dataContext.Query<MessageModerationRule>()
            ...
            .AnyAsync(ct);
        if (duplicate)
            return Result<CommunicationsModerationRuleResponse>.Conflict("A moderation rule with this name already exists.");
        ...
        if (!save.IsSuccess)
            return Result<CommunicationsModerationRuleResponse>.Failure(save.Message ?? "Moderation rule could not be created.", save.StatusCode);

        return Result<CommunicationsModerationRuleResponse>.Success(ToRuleResponse(rule), 201, "Moderation rule created.");
    }

    public async Task<Result<CommunicationsModerationRuleResponse>> UpdateRuleAsync(
        UpdateCommunicationsModerationRuleRequest request,
        CancellationToken ct = default)
    {
        var tenant = ResolveAdminTenant(request.Metadata);
        ...
        if (rule is null)
            return Result<CommunicationsModerationRuleResponse>.NotFound("Moderation rule not found.");
        ...
        var save = await dataContext.SaveChangesAsync(ct);
        if (!save.IsSuccess)
            return Result<CommunicationsModerationRuleResponse>.Failure(save.Message ?? "Moderation rule could not be updated.", save.StatusCode);

        return Result<CommunicationsModerationRuleResponse>.Success(ToRuleResponse(rule), "Moderation rule updated.");
    }
    ...
    public async Task<Result<CommunicationsReportWorkflowResponse>> ReviewReportAsync(
        ReviewMessageReportRequest request,
        CancellationToken ct = default)
    {
        var tenant = ResolveAdminTenant(request.Metadata);
        ...
        var report = await dataContext.Query<MessageReport>()
            ...
            .FirstOrDefaultAsync(ct);
        if (report is null)
            return Result<CommunicationsReportWorkflowResponse>.NotFound("Message report not found.");
        ...
        var save = await dataContext.SaveChangesAsync(ct);
        if (!save.IsSuccess)
            return Result<CommunicationsReportWorkflowResponse>.Failure(save.Message ?? "Message report could not be updated.", save.StatusCode);

        var audits = await LoadAuditAsync(report.Id, tenant.Data!.TenantId, ct);
        return Result<CommunicationsReportWorkflowResponse>.Success(new()
        {
            ReportId = report.Id,
    ...
    private async Task<List<CommunicationsReportAuditResponse>> LoadAuditAsync(Guid reportId, Guid tenantId, CancellationToken ct)
    {
        ...
        return rows.Select(audit => new CommunicationsReportAuditResponse
        {
            Id = audit.Id,
            Action = audit.Action,
    ...
    private static CommunicationsModerationRuleResponse ToRuleResponse(MessageModerationRule rule) => new()
    {
        Id = rule.Id,
        Name = rule.Name,
