...
public sealed record CommunicationsPolicySnapshot(
...
public interface ICommunicationsPolicyService
{
    Task<CommunicationsPolicySnapshot> GetPolicyAsync(Guid tenantId, CancellationToken ct = default);
    void Invalidate(Guid tenantId);
}

public sealed class CommunicationsPolicyService(
    IDataContext dataContext,
    IMemoryCache cache) : ICommunicationsPolicyService
{
    private static readonly TimeSpan CacheDuration = TimeSpan.FromMinutes(5);

    public async Task<CommunicationsPolicySnapshot> GetPolicyAsync(Guid tenantId, CancellationToken ct = default)
    {
        if (tenantId == Guid.Empty)
            throw new ArgumentException("Tenant id is required.", nameof(tenantId));

        var cacheKey = CacheKey(tenantId);
        if (cache.TryGetValue(cacheKey, out CommunicationsPolicySnapshot? policy) && policy is not null)
            return policy;
        ...
        policy = new CommunicationsPolicySnapshot(
    ...
    private static string CacheKey(Guid tenantId) => $"communications:policy:{tenantId:N}";
    ...
    private static string GetValue(IReadOnlyDictionary<string, string?> values, string key)
    {
        ...
        return CommunicationsSettingsCatalog.Definitions
