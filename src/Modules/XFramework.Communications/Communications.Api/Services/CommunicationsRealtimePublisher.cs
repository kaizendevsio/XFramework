...
public sealed class CommunicationsRealtimePublisher(
    DbContext dbContext,
    IMessageBusWrapper messageBus,
    ILogger<CommunicationsRealtimePublisher> logger) : ICommunicationsRealtimePublisher
{
    public async Task PublishAsync(MessageOutboxEvent outboxEvent, CancellationToken ct = default)
    {
        ...
        {
            logger.LogDebug("Communications outbox event {EventId} has no realtime recipients", outboxEvent.Id);
            return;
        }

        var envelope = new CommunicationsRealtimeEvent
        {
