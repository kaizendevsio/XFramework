...
public sealed class CommunicationsTemplateService(
    IDataContext dataContext,
    ITenantResolver tenantResolver,
    ICommunicationsRequestContextResolver requestContextResolver,
    ILogger<CommunicationsTemplateService> logger) : ICommunicationsTemplateService
{
    ...
    private async Task EnsureTenantTemplatesAsync(Guid tenantId, CancellationToken ct)
    {
        ...
        foreach (var systemTemplate in CommunicationsTemplateCatalog.SystemTemplates)
        {
        ...
        if (!saveResult.IsSuccess)
        {
            logger.LogWarning("Communications templates could not be seeded for tenant {TenantId}: {Message}", tenantId, saveResult.Message);
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        var systemTemplate = CommunicationsTemplateCatalog.FindSystemTemplate(templateKey);
        ...
        CancellationToken ct)
    {
        ...
        if (type == MessageTemplateTypes.System)
            errors["TemplateType"] = ["System templates are seeded by Communications and cannot be created through the API."];
    ...
    private async Task<Result<TemplateAccessContext>> ResolveTemplateAccessAsync(RequestMetadata? metadata)
    {
        ...
        if (!userContext.IsSuccess)
        {
            return Result<TemplateAccessContext>.Failure(
                userContext.Message ?? adminContext.Message ?? "Communications templates require an authenticated context",
