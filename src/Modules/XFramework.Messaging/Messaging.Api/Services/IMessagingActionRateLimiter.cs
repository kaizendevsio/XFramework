...
public static class MessagingRateLimitActions
{
...
public interface IMessagingActionRateLimiter
{
    Result Check(
        Guid tenantId,
        Guid credentialId,
...
public sealed class MessagingActionRateLimiter : IMessagingActionRateLimiter
{
        ...
        lock (counter)
        {
            ...
            if (counter.Count >= permitLimitPerMinute)
            {
                ...
                return Result.Failure(
                    $"Messaging rate limit exceeded for {action}. Retry after {retryAfterSeconds} seconds.",
                    429);
            }
