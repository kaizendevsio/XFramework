...
namespace Messaging.Integration.Clients;

public sealed record MessagingChatActor(
    Guid TenantId,
    Guid CredentialId,
    string? DeviceId = null,
    string? AccessToken = null);

public interface IMessagingChatActorProvider
{
    ValueTask<MessagingChatActor?> GetCurrentActorAsync(CancellationToken ct = default);
}

internal sealed class EmptyMessagingChatActorProvider : IMessagingChatActorProvider
{
    public ValueTask<MessagingChatActor?> GetCurrentActorAsync(CancellationToken ct = default) =>
        ValueTask.FromResult<MessagingChatActor?>(null);
}

public interface IMessagingChatClient
{
    IMessagingChatSession For(Guid tenantId, Guid credentialId, string? deviceId = null);
    ValueTask<IMessagingChatSession> ForCurrentActorAsync(string? deviceId = null, CancellationToken ct = default);
}

public interface IMessagingChatSession
{
    Guid TenantId { get; }
...
public sealed class MessagingChatClient(
    IMessagingServiceWrapper wrapper,
    IConfiguration configuration,
    IMessagingChatActorProvider actorProvider) : IMessagingChatClient
{
    public IMessagingChatSession For(Guid tenantId, Guid credentialId, string? deviceId = null)
    {
        if (!ExplicitActorSelectionAllowed())
        {
            throw new InvalidOperationException(
                "Explicit Messaging chat actor selection is disabled. Configure an IMessagingChatActorProvider and call ForCurrentActorAsync, or enable MessagingChatClient:AllowExplicitActorSelection only in trusted backend services.");
        }
        ...
        return new MessagingChatSession(wrapper, tenantId, credentialId, NormalizeDeviceId(deviceId));
    }

    public async ValueTask<IMessagingChatSession> ForCurrentActorAsync(string? deviceId = null, CancellationToken ct = default)
    {
        ...
        {
            throw new InvalidOperationException(
                "Messaging chat actor context could not be resolved for the current user.");
        }

        return new MessagingChatSession(
            wrapper,
            actor.TenantId,
            actor.CredentialId,
    ...
    private bool ExplicitActorSelectionAllowed() =>
        IsEnabled(configuration["MessagingChatClient:AllowExplicitActorSelection"]) ||
        IsEnabled(configuration["Messaging:ChatClient:AllowExplicitActorSelection"]);

    private static bool IsEnabled(string? value) =>
        bool.TryParse(value, out var enabled) && enabled;
}

internal sealed class MessagingChatSession(
    IMessagingServiceWrapper wrapper,
    Guid tenantId,
    Guid credentialId,
    string deviceId,
    string? accessToken = null,
    Func<CancellationToken, ValueTask<string?>>? accessTokenProvider = null) : IMessagingChatSession
{
    ...
    public Task SubscribeUserEventsAsync(
        Func<MessagingRealtimeEvent, Task> handler,
        CancellationToken ct = default) =>
        wrapper.SubscribeUserMessagingEventsForDeviceAsync(TenantId, CredentialId, DeviceId, handler, GetAccessTokenAsync, ct);
    ...
    public Task PublishTypingAsync(Guid threadId, bool isTyping, CancellationToken ct = default) =>
        wrapper.PublishMessagingTypingAsync(Prepare(new PublishMessagingTypingRequest
        {
            ThreadId = threadId,
            IsTyping = isTyping
        }), ct);
    ...
    public Task PublishPresenceAsync(bool isOnline, CancellationToken ct = default) =>
        wrapper.PublishMessagingPresenceAsync(Prepare(new PublishMessagingPresenceRequest
        {
            IsOnline = isOnline
        }), ct);
