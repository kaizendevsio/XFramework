...
public sealed class BoltServer
{
    ...
    private async Task HandleRequestAsync(BoltHubConnection caller, byte[] buffer, int length, CancellationToken ct)
    {
        ...
        if (_localHandlers.Count > 0 && StreamFlowCodec.TryReadRequest(span, out var frame, out var consumed))
        {
        ...
        if (!StreamFlowCodec.TryReadRequestHeader(span, out var requestId, out var recipientHash, out var totalSize))
        {
