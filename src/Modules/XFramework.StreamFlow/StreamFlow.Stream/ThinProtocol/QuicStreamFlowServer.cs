...
/// <summary>
/// QUIC-based StreamFlow server. Each RPC uses its own bidirectional QUIC stream,
/// eliminating head-of-line blocking between concurrent requests.
///
/// Same wire protocol as the thin WebSocket server (StreamFlowCodec), different transport.
/// QUIC provides: 0-RTT reconnection, per-stream congestion control, built-in TLS 1.3.
/// </summary>
public sealed class QuicStreamFlowServer : IAsyncDisposable
{
    private readonly ILogger<QuicStreamFlowServer> _logger;
    private readonly ConcurrentDictionary<string, QuicStreamFlowConnection> _connectionsByStreamId = new();
    private readonly ConcurrentDictionary<int, ConcurrentBag<QuicStreamFlowConnection>> _connectionsByServiceHash = new();
    private readonly ConcurrentDictionary<Guid, (QuicStreamFlowConnection Caller, QuicStream CallerStream, long Timestamp)> _pendingInvocations = new();
    ...
    public QuicStreamFlowServer(ILogger<QuicStreamFlowServer> logger)
    {
        _logger = logger;
    ...
    public async Task StartAsync(IPEndPoint endpoint, X509Certificate2? certificate = null, CancellationToken ct = default)
    {
        ...
        _logger.LogInformation("QUIC StreamFlow server listening on {Endpoint}", endpoint);
    ...
    {
        var connection = new QuicStreamFlowConnection(quicConnection);
    ...
    private async Task HandleRegistrationAsync(QuicStreamFlowConnection connection, QuicStream stream, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                if (StreamFlowCodec.TryReadRegister(buffer.AsSpan(0, read), out var clientId, out var clientName, out _))
                {
                    ...
                    _connectionsByServiceHash.AddOrUpdate(
                        ...
                        _ => new ConcurrentBag<QuicStreamFlowConnection> { connection },
    ...
    private async Task HandleStreamAsync(QuicStreamFlowConnection connection, QuicStream stream, CancellationToken ct)
    {
    ...
    private async Task HandleRequestAsync(QuicStreamFlowConnection caller, QuicStream callerStream,
    ...
    private QuicStream? GetOrCreateRecipientStream(QuicStreamFlowConnection recipient)
    {
    ...
    private QuicStreamFlowConnection? GetRecipient(int serviceHash)
    {
    ...
    private void RemoveConnection(QuicStreamFlowConnection connection)
    {
        if (connection.ClientId is not null)
        {
            ...
            {
                var updated = new ConcurrentBag<QuicStreamFlowConnection>(bag.Where(c => c.Id != connection.Id));
...
public sealed class QuicStreamFlowConnection
{
    ...
    public QuicStreamFlowConnection(QuicConnection connection) => QuicConnection = connection;
}
