...
public class AuthService : IAuthService
{
        ...
        CancellationToken ct = default)
    {
        try
        {
            var credential = await _dbContext.IdentityCredentials
                .FirstOrDefaultAsync(c => c.Id == request.Model.Id, ct);
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var credential = await _dbContext.Set<IdentityCredential>()
                .Include(i => i.IdentityContacts)
                .ThenInclude(i => i.Type)
                .AsSplitQuery()
                .FirstOrDefaultAsync(u => u.Id == request.CreadentialId, ct);
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var user = await _dbContext.Set<IdentityCredential>()
                .FirstOrDefaultAsync(u => u.Id == request.CredentialId, ct);
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var verificationType = await _dbContext.Set<IdentityVerificationType>()
                .FirstOrDefaultAsync(i => i.Id == request.Model.VerificationTypeId, ct);
            ...
            var identityCredential = await _dbContext.Set<IdentityCredential>()
                .Include(i => i.IdentityContacts)
                .ThenInclude(i => i.Type)
                .AsSplitQuery()
                .FirstOrDefaultAsync(i => i.Id == request.Model.CredentialId, ct);
            ...
            switch (verificationType.Name)
            {
                case nameof(IdentityConstants.VerificationType.Sms):
                    var messageTemplate = await _dbContext.Set<RegistryConfiguration>()
                    ...
                    var contact = identityCredential.IdentityContacts?
                        .FirstOrDefault(i => i.Type.Name == "Phone")?.Value;
                    ...
                    _dbContext.IdentityVerifications.Add(verification);
                    await _dbContext.SaveChangesAsync(ct);
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var identityCredential = await _dbContext.IdentityCredentials
                .Include(i => i.IdentityContacts)
                .ThenInclude(i => i.Type)
                .AsSplitQuery()
                .FirstOrDefaultAsync(
                    i => i.Id == request.CredentialId && i.TenantId == tenant.Id, ct);
            ...
            var verificationType = await _dbContext.IdentityVerificationTypes
                .AsNoTracking()
                .FirstOrDefaultAsync(i => i.Id == request.VerificationTypeId, ct);
            ...
            var anyVerification = await _dbContext.IdentityVerifications
            ...
            var lastVerification = await _dbContext.IdentityVerifications
                .Where(i => i.VerificationTypeId == verificationType.Id)
                .Where(i => i.CredentialId == identityCredential.Id)
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var storageFileType = await _dbContext.Set<StorageFileType>()
                .FirstOrDefaultAsync(i => i.Id == request.Model.TypeId, ct);
            ...
            var fileIdentifier = await _dbContext.Set<StorageFileIdentifier>()
                .FirstOrDefaultAsync(i => i.Id == request.Model.StorageFileIdentifierId, ct);
            ...
            var connectionConfig = await _dbContext.Set<RegistryConfigurationGroup>()
        ...
        CancellationToken ct)
    {
        ...
        switch (authorizationType)
        {
            case AuthorizationType.Default:
                ...
                var getDefaults = await _dbContext.RegistryConfigurations
                    .AsNoTracking()
                    .FirstOrDefaultAsync(
                        i => i.TenantId == tenant.Id && i.Key == "DefaultAuthorizeBy", ct);
            ...
            case AuthorizationType.UsernameEmailPhone:
                // Try username first
                result = await _dbContext.IdentityCredentials
                    .Include(i => i.IdentityInfo)
                    .Include(i => i.IdentityRoles)
                    .AsNoTracking()
                    .FirstOrDefaultAsync(
                        i => i.TenantId == tenant.Id && i.UserName == request.UserName, ct);

                // Try email if username not found
                result ??= await _dbContext.IdentityContacts
            ...
            case AuthorizationType.Token:
                result = await _dbContext.IdentityCredentials
                    ...
                    .Include(i => i.IdentityInfo)
                    .AsNoTracking()
                    .FirstOrDefaultAsync(i => i.UserName == request.UserName, ct);
                break;
    ...
    private async Task<List<IdentityRole>?> GetRoleList(
        ...
        CancellationToken ct)
    {
        var roleList = await _dbContext.IdentityRoles
            .AsNoTracking()
            .Include(i => i.Type)
        ...
        Guid? sessionId)
    {
        ...
        _dbContext.AuthorizationLogs.Add(authorizationLog);
    }
        ...
        AuthorizationType authorizationType)
    {
        ...
        if (authorizationType is not AuthorizationType.Token)
        {
            ...
            {
                var userSessionTypeId = await _dbContext.SessionTypes
        ...
        else
        {
            ...
            {
                var serviceSessionTypeId = await _dbContext.SessionTypes
                    .Where(i => i.TenantId == tenantId)
