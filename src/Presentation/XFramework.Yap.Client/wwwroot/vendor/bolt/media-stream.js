import { writeMediaFrame, writeMediaFeedback, writeNackRequest, MediaFrameFlags, QualityHint, } from './protocol.js';
...
export class BoltBrowserMediaStream {
    ...
    onBitrateChange;
    constructor(ws, streamId, callId, isAudio, sendWireFrame) {
        this.ws = ws;
     ...
     */
    async sendFrame(encodedData, isKeyframe = false) {
        if (this.disposed || this.ws.readyState !== WebSocket.OPEN)
            throw new Error('Media stream is closed');
     ...
     */
    async enqueueFrame(seq, timestamp, data, flags) {
        if (this.disposed)
            return;
        seq >>>= 0;
        timestamp >>>= 0;
        ...
        if (this.encryptionRequired && !isEncrypted)
            return;
    ...
    enqueueFecFrame(groupStart, groupSize, payload) {
        if (this.disposed || groupSize < 2 || groupSize > MAX_FEC_GROUP_SIZE)
            return;
        groupStart >>>= 0;
        const lengthsSize = groupSize * 4;
        if (payload.byteLength < lengthsSize)
            return;
        ...
        for (let i = 0; i < groupSize; i++) {
            ...
            if (length < 0 || length > payload.byteLength - lengthsSize)
                return;
            lengths.push(length);
        }
        ...
        while (this.fecGroups.size > MAX_FEC_GROUPS) {
            const oldest = this.fecGroups.keys().next().value;
            if (oldest === undefined)
                break;
    ...
    sendFeedback() {
        if (this.disposed || this.highestSeqReceived === 0 || this.ws.readyState !== WebSocket.OPEN)
            return;
        const jitterX100 = Math.round(this.jitterSmoothed * 100);
        const hint = this.determineQualityHint();
        const frame = writeMediaFeedback(this.streamId, this.highestSeqReceived, this.cumulativeLost, jitterX100, 0, // RTT not measured client-side
        hint);
        this.sendWireFrame(frame);
    }
    determineQualityHint() {
        if (this.highestSeqReceived > 0) {
            ...
            if (lossRate > 0.03 || this.jitterSmoothed > 50)
    ...
    async tryFecRecovery(groupStart) {
        ...
        for (let i = 0; i < group.groupSize; i++) {
            ...
            if (!this.fecFrames.has(sequence))
                missing.push(sequence);
        }
        if (missing.length !== 1)
            return; // Can only recover exactly 1 frame
    ...
    pruneFecState() {
        ...
        for (const [sequence, frame] of this.fecFrames) {
            if (frame.createdAt <= cutoff)
                this.fecFrames.delete(sequence);
        }
        for (const [groupStart, group] of this.fecGroups) {
            if (group.createdAt <= cutoff)
                this.fecGroups.delete(groupStart);
        }
    }
}
