...
The sign-in rolls rather than running down a fixed timer, because an installed messenger must not sign you out while you are using it. Any authenticated request pushes the idle deadline ninety days out again, and there is no absolute cap: a device that keeps opening Yap stays signed in until it signs out, its upstream session is revoked, or its credential changes. The cookie and the server-side session entry carry the same deadline and are renewed together from `OnValidatePrincipal`, and the store entry's TTL tracks that deadline so Redis cannot evict a session the app still honours. Both sign-in paths (OPAQUE and the legacy password form) ask IdentityServer for a `PersistentSession`, which creates the upstream session with no `ExpiresAt`; without it every sign-in got the 24-hour default cap that no refresh extends, and the first refresh after that signed the device out. The one upstream bound left is the refresh token, which for a persistent session lives 120 days and slides on each rotation (other clients keep the configured fourteen); the ninety-day idle window sits inside it. Sign-out still revokes locally and upstream immediately, and a longer session changes nothing about message encryption, which stays gated per device.
...
Subscriptions are owned by the Notifications module (`Notifications.NotificationPushSubscription`), not by this host, which keeps no database. The browser endpoint, its `p256dh` public key, and its `auth` secret are stored per device per credential, unique on `(TenantId, sha256(endpoint))`. A push service replying 404 or 410 deletes the row immediately rather than retrying a dead endpoint forever, and logs the removal at Information.

**The device repairs its own registration.** The server row is what delivers, so a deleted row used to mean silence until someone toggled Settings, while the browser still held a subscription and Settings still read "On". Now the foreground presence heartbeat (`push.js`) doubles as a check: a `404` from `/api/chat/push/presence`, or a granted permission with no browser subscription, re-registers this device for the signed-in account without prompting. An endpoint this device had registered for the same account and the server has since dropped was reported gone by the push service, so it is replaced with a fresh subscription rather than resent. Repair never runs without an existing `granted` permission, never on the way to the background, and never after the person turned push off on this device (a per-account flag in `localStorage`). The Settings toggle reports this server-side state via `yap.push.ensure`.
