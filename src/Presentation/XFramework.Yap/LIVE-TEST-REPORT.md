...
Deployment `34588648781` completed successfully at develop `041bc347`; all 13 application containers were healthy. Yap ran from the branch containing that develop revision and its application changes (PR #445, draft). Its 30 tests passed locally and in Phase 0 CI `34588972468` on `ff8d2e0e`. The browser request-header-policy/Statsig timeout recovered through normal CUA calls without configuration changes or resetting the session.

At approximately `10:54–10:59Z`, three ordinary test accounts signed into separate `bob.localhost`, `carol.localhost`, and `dave.localhost` Chrome sessions:

- **Group typing:** all six sender/recipient directions passed for both start and stop. With Bob and Carol typing simultaneously, stopping Bob cleared Carol's indicator while Dave still saw Carol typing; stopping Carol cleared the remaining indicators. Bob's immediate restart appeared for both peers. Indicators also expired after inactivity. The browser tool's empty-string fill did not change textarea values; these stop checks used verified whitespace-only input, which the composer treats as blank.
- **Subscription isolation and renewal:** after the normal service reconnect at `10:56:53Z`, Bob's typing still reached Carol and Dave without reload. Switching Carol to the direct conversation did not cancel the Bob/Dave group subscription. Direct typing worked in both directions and did not appear in Dave's group.
- **Reply composer:** Bob's direct-thread typing appeared for Carol; sending cleared the indicator. The new `Yap final thread typing and reply 1100` reply appeared in both users' thread views, increasing the count from two to three.
- **Small attachment:** Bob uploaded `live-chat-attachment.txt` (50 bytes) to the group. Carol and Dave received the message. Carol's Attachments action produced an authorized link on the reachable `xeon-dev:9000` host; opening the unchanged link in Chrome displayed the expected text. An independent HTTP download of that exact UI-provided URL matched the original 50 bytes and SHA-256 `6535DCA266A98B6D41C9374C0545EDBF0D3244F8DEEF8B8CA3B149FA52586A84`.
- **Multipart attachment:** Dave uploaded `live-chat-multipart.txt` (6,622,600 bytes), and Bob and Carol received the message. Bob's authorized link opened the complete benign text in Chrome. An independent raw download of the exact UI-provided URL matched the original length and SHA-256 `9CBD674EE3A238A7D4B82AB6A3159875FF9041A7182E7AA2702C36AD3B9EBB83`. Browser rendering and raw download integrity were checked separately; signed URLs are not retained in this report.
- **Direct attachment and privacy:** Bob sent the small file in `Yap final private attachment 1100`; Carol received it and obtained an authorized download link. Dave navigated to that direct conversation and received a permissions error, with no private messages rendered; the matching `GetThreadRequest` returned 403 at `10:59:45Z`.
- **Further persistence/recovery:** Carol added Love to the private attachment message. Bob saw count 1 with his pressed state false, while Carol's was true. Bob reloaded after the `10:58:53Z` and `11:00:53Z` transport renewals; the reaction remained and his fresh direct typing still reached Carol.
- **Search authorization:** the same mixed-case global search, `PrIvAtE AtTaChMeNt 1100`, returned one matching private message for Bob and zero results for Dave. Removing Carol's reaction removed the chip.
- **Normal browser controls:** after tooling recovery, ordinary Playwright role-based clicks opened and closed Search, and Carol's normal Send message click delivered `Yap Chrome click delivery 1106` to Bob. Assertions ran after Blazor rendered asynchronously. This supersedes the earlier blanket limitation that normal clicks were unavailable; earlier scenarios still used the documented DOM activation method.
